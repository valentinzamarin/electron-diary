<template>
  <aside class="aside">
    <div class="flex flex-col gap-4 h-screen pt-4 pb-4">
      <div class="border-b-[1px] border-slate-200 px-4 pb-4">
        <input
          type="text"
          v-model="searchQuery"
          placeholder="Найти..."
      ...
      <div v-if="filteredPosts.length > 0" v-for="item in filteredPosts" :key="item.id" class="border-b-[1px] border-slate-200 cursor-pointer" @click="openPost(item.id)">
        <div class="px-4 pb-4">
          {{ item.title }}
